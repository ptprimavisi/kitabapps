...
// Component
@Component({
	selector: 'page-kitab_detil-list',
	templateUrl: 'kitab_detillist.html',
	styleUrls: ['kitab_detillist.scss']
})
export class kitab_detilListPage implements OnDestroy, OnInit {
	...
	keyCount: number = 1;
	pageId: string = "list";
	pageUrl: string = "kitab_detillist";
	isLoggedIn: boolean;
	isAdmin: boolean;
	userId: string;
	permissions: any;
	...
	getForeignKey(): any {
		let keys, key;
		keys = {};
		key = this.activatedRoute.snapshot.paramMap.get("pid");
		if (!this.dbapp.isEmpty(key))
			keys["pid"] = key;
		if (Object.keys(keys).length === 1) {
			keys[this.dbapp.TABLE_MASTER] = "kitab";
			return keys;
		}
		return null;
	}
	...
	async init() {
		...
		await this.user.set("kitab_detil_foreignKey", this.foreignKey);
