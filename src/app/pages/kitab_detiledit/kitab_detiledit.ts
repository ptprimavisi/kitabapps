...
import { kitab_detil } from '../../providers';

// Component
@Component({
	selector: 'page-kitab_detil-edit',
	templateUrl: 'kitab_detiledit.html',
	styleUrls: ['kitab_detiledit.scss']
})
export class kitab_detilEditPage implements OnDestroy, OnInit {
	@ViewChild("pid", { static: false }) pid: IonicSelectableComponent;
	@ViewChild("aktif", { static: false }) aktif: IonicSelectableComponent;
	@ViewChild("gambar", { static: false }) gambar: FileUploadComponent;
	key: any;
	keyCount: number = 1;
	item: DbRecord;
	...
	foreignKey: any;
	pageId: string = "edit";
	pageUrl: string = "kitab_detiledit";
	returnUrl: string = "/kitab_detillist";
	...
	constructor(
		...
		public items: kitab_detil,
		) {
		this.key = this.getPrimaryKey();
		this.item$ = new BehaviorSubject<DbRecord>(null);
		this.userIdAllowed = this.dbapp.userIdAllow("kitab_detil", this.pageId);
		this.getUserData();
	}
	...
	async init() {
		...
		this.formGroup = this.formBuilder.group({
			...
			pid: [{ value: "", disabled: false }, []],
			judul: [{ value: "", disabled: false }, []],
			isi: [{ value: "", disabled: false }, []],
			gambar: [{ value: [], disabled: false }, [FileUploadValidators.accept([".gif",".jpg",".jpeg",".bmp",".png",".doc",".docx",".xls",".xlsx",".pdf",".zip"])]],
			fn_gambar: [{ value: [], disabled: false }],
			keterangan: [{ value: "", disabled: false }, []],
			tag: [{ value: "", disabled: false }, []],
			rujukan: [{ value: "", disabled: false }, []],
			aktif: [{ value: "", disabled: false }, []],
		}, {
			validators: [
			]
		});
		this.f.gambar.valueChanges.subscribe(files => {
			if (files) {
				if (!this.gambar.control.isMultiple)
					this.removeFiles("gambar");
				this.item["gambar"].addFiles(files);
			}
		});
		await this.getItem();
		if (this.item) {
			this.f.fn_gambar.setValue(this.item["gambar"].files);
			this.item["gambar"].valueChanges.subscribe(value =>
				this.f.fn_gambar.setValue(this.item["gambar"].files));
		}
	}
			...
			try {
				...
				if (this.dbapp.isObject(item)) {
					...
					this.formGroup.patchValue({
						...
						pid: this.item['pid'].formValue,
						judul: this.item['judul'].dbValue,
						isi: this.item['isi'].dbValue,
						fn_gambar: this.item['gambar'].files,
						keterangan: this.item['keterangan'].dbValue,
						tag: this.item['tag'].dbValue,
						rujukan: this.item['rujukan'].dbValue,
						aktif: this.item['aktif'].formValue
					});
				}
			} catch(err) {
				this.showError(err);
	...
	async edit() {
		...
		if (!this.formGroup.valid) {
			...
			this.saving = true;
			try {
				res = await this.items.edit(this.key, value);
			} finally {
				this.saving = false;
			}
	 ...
	 */
	async getForeignKey() {
		return await this.user.get("kitab_detil_foreignKey") || {};
	}
	...
	async unformatValue(value: any) {
		...
		for (let fldvar of ["id","pid"])
			value[fldvar] = this.dbapp.parseNumber(value[fldvar], gpSep, decSep);
		for (let fldvar of ["gambar"]) {
			value[fldvar] = this.item[fldvar].files.filter(f => f.file); // Submit new files only
			value["fn_" + fldvar] = this.item[fldvar].value; // Submit file names only
		}
		return value;
	}
