...
// Component
@Component({
	selector: 'page-userlevels-edit',
	templateUrl: 'userlevelsedit.html',
	styleUrls: ['userlevelsedit.scss']
})
export class userlevelsEditPage implements OnDestroy, OnInit {
	key: any;
	keyCount: number = 1;
	item: DbRecord;
	...
	foreignKey: any;
	pageId: string = "edit";
	pageUrl: string = "userlevelsedit";
	returnUrl: string = "/userlevelslist";
	...
	async init() {
		...
		this.formGroup = this.formBuilder.group({
			userlevelid: [{ value: "", disabled: false }, [Validators.required]],
			userlevelname: [{ value: "", disabled: false }, [Validators.required]],
		});
		await this.getItem();
	}
			...
			try {
				...
				if (this.dbapp.isObject(item)) {
					...
					this.formGroup.patchValue({
						...
						userlevelname: this.item['userlevelname'].dbValue
					});
				}
			} catch(err) {
				this.showError(err);
	 ...
	 */
	async getForeignKey() {
		return await this.user.get("userlevels_foreignKey") || {};
	}
