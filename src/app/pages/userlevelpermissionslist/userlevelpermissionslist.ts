...
// Component
@Component({
	selector: 'page-userlevelpermissions-list',
	templateUrl: 'userlevelpermissionslist.html',
	styleUrls: ['userlevelpermissionslist.scss']
})
export class userlevelpermissionsListPage implements OnDestroy, OnInit {
	...
	keyCount: number = 2;
	pageId: string = "list";
	pageUrl: string = "userlevelpermissionslist";
	isLoggedIn: boolean;
	isAdmin: boolean;
	userId: string;
	permissions: any;
	...
	async init() {
		...
		await this.user.set("userlevelpermissions_foreignKey", this.foreignKey);
	...
	getPrimaryKey(item: any): any {
		...
		if (item) {
			keys = {};
			if (item["userlevelid"])
				keys["userlevelid"] = item["userlevelid"].value;
			if (item["tablename"])
				keys["tablename"] = item["tablename"].value;
