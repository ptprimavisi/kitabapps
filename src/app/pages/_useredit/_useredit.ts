...
// Component
@Component({
	selector: 'page-_user-edit',
	templateUrl: '_useredit.html',
	styleUrls: ['_useredit.scss']
})
export class _userEditPage implements OnDestroy, OnInit {
	@ViewChild("level", { static: false }) level: IonicSelectableComponent;
	...
	foreignKey: any;
	pageId: string = "edit";
	pageUrl: string = "_useredit";
	returnUrl: string = "/_userlist";
			...
			try {
				...
				if (this.dbapp.isObject(item)) {
					...
					this.formGroup.patchValue({
						...
						nama: this.item['nama'].dbValue,
						username: this.item['username'].dbValue,
						password: this.item['password'].dbValue,
						keterangan: this.item['keterangan'].dbValue,
						level: this.item['level'].formValue,
	 ...
	 */
	async getForeignKey() {
		return await this.user.get("_user_foreignKey") || {};
	}
	...
	async unformatValue(value: any) {
		...
		for (let fldvar of ["id","level","pid"])
