...
// Component
@Component({
	selector: 'page-kitab-edit',
	templateUrl: 'kitabedit.html',
	styleUrls: ['kitabedit.scss']
})
export class kitabEditPage implements OnDestroy, OnInit {
	@ViewChild("aktif", { static: false }) aktif: IonicSelectableComponent;
	@ViewChild("sampul", { static: false }) sampul: FileUploadComponent;
	key: any;
	keyCount: number = 1;
	item: DbRecord;
	...
	foreignKey: any;
	pageId: string = "edit";
	pageUrl: string = "kitabedit";
	returnUrl: string = "/kitablist";
	...
	async init() {
		...
		this.formGroup = this.formBuilder.group({
			...
			pengarang: [{ value: "", disabled: false }, []],
			keterangan: [{ value: "", disabled: false }, []],
			sampul: [{ value: [], disabled: false }, [FileUploadValidators.accept([".gif",".jpg",".jpeg",".bmp",".png",".doc",".docx",".xls",".xlsx",".pdf",".zip"])]],
			fn_sampul: [{ value: [], disabled: false }],
			thn_terbit: [{ value: "", disabled: false }, [DbAppValidators.integer]],
		...
		this.f.sampul.valueChanges.subscribe(files => {
			if (files) {
				if (!this.sampul.control.isMultiple)
					this.removeFiles("sampul");
				this.item["sampul"].addFiles(files);
			}
		});
		await this.getItem();
		if (this.item) {
			this.f.fn_sampul.setValue(this.item["sampul"].files);
			this.item["sampul"].valueChanges.subscribe(value =>
				this.f.fn_sampul.setValue(this.item["sampul"].files));
		}
	}
			...
			try {
				...
				if (this.dbapp.isObject(item)) {
					...
					this.formGroup.patchValue({
						...
						pengarang: this.item['pengarang'].dbValue,
						keterangan: this.item['keterangan'].dbValue,
						fn_sampul: this.item['sampul'].files,
						thn_terbit: this.item['thn_terbit'].value,
	 ...
	 */
	async getForeignKey() {
		return await this.user.get("kitab_foreignKey") || {};
	}
