...
class _user extends DbTable
{
	...
	public function __construct()
	{
		...
		// id
		$this->id = new DbField('_user', 'user', 'id', 'id', '`id`', '`id`', 3, 11, -1, FALSE, '`id`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		...
		// nama
		$this->nama = new DbField('_user', 'user', 'nama', 'nama', '`nama`', '`nama`', 200, 255, -1, FALSE, '`nama`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->nama->Sortable = FALSE; // Allow sort
		$this->fields['nama'] = &$this->nama;

		// username
		$this->username = new DbField('_user', 'user', 'username', 'username', '`username`', '`username`', 200, 255, -1, FALSE, '`username`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->username->Required = TRUE; // Required field
		$this->username->Sortable = FALSE; // Allow sort
		$this->fields['username'] = &$this->username;

		// password
		$this->password = new DbField('_user', 'user', 'password', 'password', '`password`', '`password`', 200, 255, -1, FALSE, '`password`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->password->Required = TRUE; // Required field
		$this->password->Sortable = FALSE; // Allow sort
		$this->fields['password'] = &$this->password;

		// keterangan
		$this->keterangan = new DbField('_user', 'user', 'keterangan', 'keterangan', '`keterangan`', '`keterangan`', 200, 255, -1, FALSE, '`keterangan`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->keterangan->Sortable = FALSE; // Allow sort
		$this->fields['keterangan'] = &$this->keterangan;

		// level
		$this->level = new DbField('_user', 'user', 'level', 'level', '`level`', '`level`', 3, 11, -1, FALSE, '`EV__level`', TRUE, TRUE, FALSE, 'FORMATTED TEXT', 'SELECT');
		$this->level->Sortable = FALSE; // Allow sort
		...
		// pid
		$this->pid = new DbField('_user', 'user', 'pid', 'pid', '`pid`', '`pid`', 3, 11, -1, FALSE, '`EV__pid`', TRUE, TRUE, FALSE, 'FORMATTED TEXT', 'SELECT');
		...
		$this->pid->Lookup = new Lookup('pid', '_user', FALSE, 'id', ["nama","","",""], [], [], [], [], [], [], '', '');
	...
	public function getSqlSelectList() // Select for List page
	{
		$select = "";
		$select = "SELECT * FROM (" .
			"SELECT *, (SELECT `userlevelname` FROM `userlevels` `TMP_LOOKUPTABLE` WHERE `TMP_LOOKUPTABLE`.`userlevelid` = `user`.`level` LIMIT 1) AS `EV__level`, (SELECT `nama` FROM `user` `TMP_LOOKUPTABLE` WHERE `TMP_LOOKUPTABLE`.`id` = `user`.`pid` LIMIT 1) AS `EV__pid` FROM `user`" .
			") `TMP_TABLE`";
	...
	protected function useVirtualFields()
	{
		...
		if (ContainsString($orderBy, " " . $this->level->VirtualExpression . " "))
			return TRUE;
