 ...
 * (C) 2002-2021 e.World Technology Limited. All rights reserved.
*/
...
class Breadcrumb
{
	...
	{
		global $Language;
		$this->Links[] = ["home", "HomePage", "", "ew-home", "", FALSE]; // Home
	}
		...
		if ($provider != "") {
			...
			if (array_key_exists($provider, $providers) && $providers[$provider]["enabled"]) {
				try {
					...
					// Note: callback url is ?provider=xxx
					if (!array_key_exists("callback", $authConfig))
						$authConfig["callback"] = FullUrl("?provider=" . $provider, "auth");
